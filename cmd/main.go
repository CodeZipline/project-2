...
func main() {
	...
	mux1.HandleFunc("/read", func(w http.ResponseWriter, r *http.Request) {
		...
		// Provide the query string for key="key".
		key := r.FormValue("key")
	...
	go func() {
		errorChan <- http.ListenAndServe(":"+os.Getenv("B_ORIGIN_PORT"), mux1)
	}()
